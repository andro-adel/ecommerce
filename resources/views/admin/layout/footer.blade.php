<!-- Footer -->
<footer class="py-4 bg-light mt-auto">
  <div class="container-fluid">
    <div class="d-flex align-items-center justify-content-between small">
      <div class="text-muted">Copyright &copy; Your Website 2020</div>
      <div>
        <a href="#">Privacy Policy</a>
        &middot;
        <a href="#">Terms &amp; Conditions</a>
      </div>
    </div>
  </div>
</footer>
</div>
</div>
<!-- End of Footer -->
<script src="{{asset('admins/js/fontAwesome.min.js')}}"></script>
<script src="{{asset('admins/js/jquery.js')}}"></script>
<script src="{{asset('admins/js/jquery.easing.min.js')}}"></script>
<script src="{{asset('admins/js/bootstrap.min.js')}}"></script>
<script src="{{asset('admins/js/select2.min.js')}}"></script>
<script src="{{asset('admins/js/backend.js')}}"></script>
<script src="{{asset('admins/js/alertify.min.js')}}"></script>
<script src="{{asset('admins/js/scripts.js')}}"></script>
<script src="https://cdn.jsdelivr.net/npm/bootstrap@4.5.3/dist/js/bootstrap.bundle.min.js" crossorigin="anonymous">
</script>
<script src="https://cdnjs.cloudflare.com/ajax/libs/Chart.js/2.8.0/Chart.min.js" crossorigin="anonymous"></script>
<script src="https://cdn.datatables.net/1.10.20/js/jquery.dataTables.min.js" crossorigin="anonymous"></script>
<script src="https://cdn.datatables.net/1.10.20/js/dataTables.bootstrap4.min.js" crossorigin="anonymous"></script>
<script src="{{asset('admins/js/chart-area-demo.js')}}"></script>
<script src="{{asset('admins/js/chart-bar-demo.js')}}"></script>
<script src="{{asset('admins/js/datatables-demo.js')}}"></script>